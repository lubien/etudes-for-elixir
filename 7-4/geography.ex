...
defimpl Valid, for: City do
  def valid?(city) do
    ...
    -90 <= city.latitude and city.latitude <= 90  and
    -180 <= city.longitude and city.longitude <= 180
  end
end
...
defimpl Inspect, for: City do
  def inspect(city, _options) do
    lat = if (city.latitude < 0) do
      to_string(Float.round(abs(city.latitude * 1.0), 2)) <> "°S"
    else
      to_string(Float.round(city.latitude * 1.0, 2)) <> "°N"
    end

    lon = if (city.longitude < 0) do
      to_string(Float.round(abs(city.longitude * 1.0), 2)) <> "°W"
    else
      to_string(Float.round(city.longitude * 1.0, 2)) <> "°E"
    end

    "#{city.name} (#{city.population}) #{lat} #{lon}"
  end
end
